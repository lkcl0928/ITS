...
class VideoTracker(object):
    ...
    def run(self):
        ...
        while self.vdo.grab():
            ...
            if len(outputs) > 0:
                ...
                for i, bbox in enumerate(bbox_xyxy):
                    ...
                    if self.idx_frame > 2:
                        for key, value in self.dict_box.items():
                            for a in range(len(value) - 1):
                                ...
                                cv2.circle(ori_im, (int(x_center), int(y_center)), 3, color, 5, 8)
                    ...
                    if (self.carPre.get(id, 0) == 0):
                        ...
                        if self.carSpeed.get(id, 0) > self.carSpeedLimit:
                            ...
                            imgg.save(self.outputDir + "illegal/overspeed/" + str(self.idx_frame) + "_" + ".jpg")
                        ...
                        if self.carTurn.get(id, 0) == 'LEFT' and (
                            ...
                            imgg.save(self.outputDir + "illegal/turnwrong/" + str(self.idx_frame) + "_" + ".jpg")
                    ...
                    if self.carDirection.get(id, 0) == 'UP' and (
                        ...
                        if len(config.laneline) > 0:
                            ...
                            if self.judge_line_illegal(id, tmp, config):
                                ...
                                imgg = Image.fromarray(cv2.cvtColor(imgTmp, cv2.COLOR_BGR2RGB))
                                imgg.save(self.outputDir + "illegal/touchline/" + str(self.idx_frame) + "_" + ".jpg")
                    ...
                    if self.carDirection.get(id, 0) == 'UP':
                        if len(config.laneline2) > 0:
                            ...
                            for j in range(len(box_xywh)):
                                x_center = box_xywh[j][0] + box_xywh[j][2] / 2  # 求框的中心x坐标
                                y_center = box_xywh[j][1] + box_xywh[j][3] / 2  # 求框的中心y坐标
                                if x_center >= self.laneline2[0] and x_center <= self.laneline2[4]:
                                    # print(tmp)
                                    x, y, xx, yy = tmp
                ...
                font = ImageFont.truetype(font='model_data/simhei.ttf',
                                          size=np.floor(0.012 * np.shape(ori_im)[1]).astype('int32'))
                ori_im = pil_draw_box(ori_im, bbox_xyxy, identities, self.carSpeed, self.carLicense, self.carDirection,font)
                ...
                if self.detector.carCnt + self.detector.busCnt + self.detector.truckCnt >= self.trafficJamLimit and self.idx_frame % 8 == 0:
                    ...
                    # ori_im = cv2.putText(ori_im, 'Traffic Jam!!!', (1200, 70), cv2.FONT_HERSHEY_COMPLEX, 3, (0, 0, 255),3)
            ...
            if len(self.trafficLine1) > 0:
                ...
                # plot_one_box(self.trafficLine2, ori_im, label="Zebra crossing2", color=(105,210,255), line_thickness=3,length=20,corner_color= (0,255,0))
            ...
            # if len(config.laneline) > 0:
            # self.plot_lane(config.laneline, ori_im, "Laneline")
            if len(config.laneline2) > 0:
                # print(config.laneline2)
                self.plot_lane(config.laneline2, ori_im, "Laneline2")
            ori_im = cv2.putText(ori_im, "carIn:%d carOut:%d" % (len(self.inCar), len(self.outCar)), (0, 150),
                                 cv2.FONT_HERSHEY_DUPLEX, 2, (255, 255, 255), 2)
            if config.SAVEVIDEO == True:
            ...
            cv2.resizeWindow('video',self.videoWidth,self.videoHeight)
            cv2.waitKey(1)
    ...
    def carrunred(self, id, x1, x2, y1, y2, ori_im):
        ...
        if (self.detector.trafficLightColor == 'red') and self.carDirection.get(id, 0) == 'UP' and self.carSpeed.get(id,
                                                                                                                     0) >= 20:
            ...
            imgg = Image.fromarray(cv2.cvtColor(imgg, cv2.COLOR_BGR2RGB))
            imgg.save(self.outputDir + "illegal/carrunred/" + str(self.idx_frame) + "_" + ".jpg")
            font = ImageFont.truetype(font='model_data/simhei.ttf',
                                      size=np.floor(0.012 * np.shape(ori_im)[1]).astype('int32'))
    ...
    def save_csv_data(self):
        ...
        if self.idx_frame % 6 == 0:
            with open(self.outputDir + "csv/count.csv", 'a', newline='') as csvfile:
                ...
                writer.writerow([int(self.idx_frame / 6), int(self.detector.carCnt), int(self.detector.personCnt),
                                 int(self.detector.motoCnt), int(self.detector.busCnt), int(self.detector.truckCnt)])
    ...
    def generate_chart(self):
        ...
        line = Line("检测目标数量分析图")
        t1 = ['time', 'car', 'person', 'moto', 'bus', 'truck']
