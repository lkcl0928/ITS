...
if __name__ == '__main__':
    ...
    parser.add_argument("-image", help='image path', default='./LPRNet/data/6.png', type=str)
    ...
    for i in range(bboxes.shape[0]):
        ...
        cv2.rectangle(image, (x1, y1), (x2, y2), (255, 66, 45), 2)
        image = cv2ImgAddText(image, labels[0], (x1, y1 - 12), textColor=(255, 0, 0), textSize=25)

        print("model inference in {:2.3f} seconds".format(time.time() - since))
        image = cv2.resize(image, (640, 989), fx=1 / args.scale, fy=1 / args.scale, interpolation=cv2.INTER_CUBIC)
